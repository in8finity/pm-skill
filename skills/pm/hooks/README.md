...
| `skip_verifier_requires_note.sh` | `PreToolUse` on `Bash` | Refuses `pm finished --skip-verifier` unless a `--note "<≥10 chars>"` justifies the bypass. Other `pm finished` invocations and non-pm commands pass through. |
...
| `PM_HOOK_ALLOW_BARE_SKIP=1` | Disables `skip_verifier_requires_note.sh`. Restores the unannotated `--skip-verifier` behavior. |
