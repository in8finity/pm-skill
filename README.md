...
2. **Register the planning schema** (once per data dir):
   ...
   ```
3. **Use the skills** — invoke through Claude Code via `planning-plan`, `planning-next`, etc., or call `pm` directly:
   ```bash
...
| At most one agent owns the latest TaskStatus of a task | `executing.py` exit 8 (post-append recheck) |
...
Both race conditions are content-addressed gates: the append-only store does the structural enforcement; the scripts plumb that property up to the operator-visible exit codes.
...
`pm execute` (the `planning-execute` skill) spawns N agents in parallel running this loop.
